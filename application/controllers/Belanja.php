	...
	public function add()
{
    ...
    $stock = $this->m_barang->get_stock($id_barang);

    // Periksa apakah jumlah yang diminta melebihi stok
    if ($qty > $stock) {
        // Set pesan error dan redirect kembali
        $this->session->set_flashdata('error', 'Jumlah yang diminta melebihi stok yang tersedia!');
        redirect($redirect_page, 'refresh');
    } else {
        // Jika stok mencukupi, tambahkan ke keranjang
        $data = array(
            'id'      => $id_barang,
            'qty'     => $qty,
